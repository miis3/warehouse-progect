...
إذا كان المنفذ 8000 مستخدمًا وفيه بيانات تجربة تريد الحفاظ عليها، افتح PowerShell جديدًا وشغّل نسخة مستقلة:

```powershell
$env:WAREHOUSE_PREVIEW_PORT='8002'
npm run preview:test
```

افتح `http://127.0.0.1:8002`. بيانات المنفذين منفصلة ومؤقتة؛ لا تمحوها إعادة تحميل الصفحة أو إعادة الدخول، لكنها تزول عند إنهاء عملية الخادم المقابلة. نتائج اختبار المتصفح موثقة في `QA_REPORT.md`.
