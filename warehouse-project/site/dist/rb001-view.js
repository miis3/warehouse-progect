...
const rbPhoto=item=>equipmentPhotos[photoKey(item)];
...
function rbPhotoCards(){
 ...
 return `<div class="rb-item" style="--slot-x:${x}%;--slot-y:${y}px;--order:${slot}"><img src="${escapeHtml(photo.src)}" alt="صورة توضيحية: ${escapeHtml(rbName(item))}" width="160" height="140"><span>${escapeHtml(rbName(item))}</span><small>${rbQuantity(item)}</small></div>`;
 }).join('');
}
function renderRb001(box){
 ...
 app.innerHTML=`<div class="topline"><div><div class="breadcrumbs">برافو / القسم ${pad(state.bay)} / لفل ${state.level}</div><h1>الصندوق <b dir="ltr">${escapeHtml(box.label)}</b></h1></div><button data-action="boxes">رجوع إلى الصناديق ←</button></div>
 ...
 <details class="rb-sources"><summary>مصادر الصور التوضيحية</summary><ul>${items.map((item,index)=>{const p=rbPhoto(item);return `<li><a href="${escapeHtml(p.source)}" target="_blank" rel="noopener noreferrer">${escapeHtml(rbName(item))}</a>${p.credit?' — '+escapeHtml(p.credit):''}</li>`;}).join('')}</ul></details>`;
}
function rbLock(locked){state.busy=locked;app.querySelectorAll('button').forEach(b=>b.disabled=locked);}
...
async function handleRbAction(d){
 if(d.action==='rb001'){Object.assign(state,{row:1,side:'A',bay:5,level:1,view:'case',box:'bravo-s5-l1-box2',open:false});rbScene.open=false;rbScene.page=0;renderBravo();return true;}
