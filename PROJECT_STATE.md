...
- يدعم محاكي الهوية المحلي الآن إنشاء حسابات الأمناء/المديرين الاختبارية وتغيير كلمات مرورهم، مع بقاء فرض الصلاحيات داخل RPC نفسه.
...
- اختبار متصفح 2026-09-20: نجحت رحلة قطعة كاملة ورحلة صندوق بقطعتين، الرفض، دخول الأمين، اعتماد الصرف والإرجاع، البحث والموقع، الصيانة، السجل، واستمرار السجل بعد تحديث الصفحة وإعادة الدخول. مُنع صرف صندوق مُعار للعامل الثاني. التفاصيل والصور في `QA_REPORT.md`.
- أثناء الاختبار ظهر خلل في إنشاء حساب الأمين بنسخة Preview القديمة؛ أُصلح واختُبر في المتصفح على منفذ مستقل، وأُضيفت له فحوص HTTP وSQL. `npm run check` و`npm test` ناجحان (`19/19`).
...
- Preview يحفظ بياناته في الذاكرة طوال عمر العملية فقط؛ تحديث الصفحة وإعادة الدخول لا يمسحانها، لكن إعادة تشغيل الخادم تمسحها. أثناء QA تُرك خادم `8000` القديم دون إعادة تشغيل لحماية بيانات تجربته، وشُغّل الإصلاح منفصلًا.
...
- لم يُختبر تحمل إعادة تشغيل الخادم أو عمل المتصفح على الهاتف أو سباقات الحمل الحقيقي على Supabase البعيد. Preview مؤقت وليس قاعدة إنتاج دائمة.
